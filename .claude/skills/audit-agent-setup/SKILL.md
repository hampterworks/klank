...
1. **Mirror parity** - every `.claude/agents/<n>.md` has a matching `.github/agents/<n>.agent.md` and vice versa. Skills are **not** mirrored: flag any `.github/agents/<n>.agent.md` that shadows a `.claude/skills/<n>/` directory (Copilot auto-discovers skills).
...
5. **Mirror alignment** - `description` and `model` are identical between `.claude/agents/<n>.md` and its `.github/agents/<n>.agent.md`.
